# Hi, I'm Kawther Merain 👋

**Software Architect Student @ Epitech Marseille | Fullstack Developer (MERN)**

I specialize in building scalable, high-performance web applications. My background is primarily in **Fintech** and **SaaS**, where I focus on system reliability, database architecture, and transforming complex business requirements into robust technical solutions.

---

### 🏗️ Technical Focus
- **Architecture:** System Design, Scalability, API Design, Producer/Consumer Patterns.
- **Fintech:** Secure payment integrations (Stripe) and financial data reliability.
- **Big Data:** Real-time data collection, processing, and visualization.

### 🛠️ Tech Stack
- **Frontend:** React.js, Next.js, Tailwind CSS, JavaScript (ES6+)
- **Backend:** Node.js, Express.js, MongoDB, MySQL
- **Tools & DevOps:** Docker, Git, Linux, Agile/Scrum

---

### 🚀 Key Projects

#### [Crypto Viz](https://github.com/KaeMrn/crypto-viz)
*Architecting a Big Data application for real-time cryptocurrency analysis using the Producer/Consumer paradigm.*

#### [Stocks](https://github.com/KaeMrn/stocks-bi)
*A Business Intelligence platform for retail inventory management with a focus on data insights and React performance.*

#### [E-Commerce Platform](https://github.com/KaeMrn/ecommerce-stripe)
*A full-scale online store focused on secure transactions and Stripe API compliance.*

---

### 📫 Connect with me
- **LinkedIn:** [linkedin.com/in/kawther-merain](https://linkedin.com/in/kawther-merain)
- **Email:** merain.kawther@gmail.com

---
